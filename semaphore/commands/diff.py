...
def diff(client: SemaphoreClient, config: dict) -> None:
    ...
    print("  Inventory:")
    diffs = _diff_inventory(config.get("inventories", []), srv_invs, key_id_to_name, repo_id_to_name)
    _print_diffs(diffs, "    ")
...
def _diff_inventory(
    ...
    for cfg in inventory_cfg:
        ...
        changes = _compare({
            ...
            "repository": (repo_id_to_name.get(srv.get("repository_id")), cfg.get("repository")),
            "ssh_key":    (key_id_to_name.get(srv.get("ssh_key_id"), "None"),    cfg.get("ssh_key", "None")),
