...
semaphore-cli export "My Project"  # save a deployed project to a config file
...
### `semaphore-cli export <project> [output.json]`

Save a deployed project to a SemaphoreUI-native export file. `<project>` is matched by numeric id or exact name. If no output path is given, the file is named after a slug of the project name. The resulting file is the same format `diff` and `apply` consume — so `export` then `diff` shows a project as fully in sync.

```
$ semaphore-cli export "Web Infrastructure"

Exported project 'Web Infrastructure' (id=1) → web-infrastructure.json
  2 keys, 2 repositories, 3 inventories, 2 environments, 7 templates
```
...
For a **`static`** inventory, the `inventory` field holds the inventory text itself. For a **`file`** inventory, the inventory is loaded from a repository at run time: `repository` names the repository (by name, as used everywhere else) and `inventory` holds the path to the inventory file within it. Both fields are required when `type` is `file`:
...
│   ├── export.py      # Export a deployed project to a config file
